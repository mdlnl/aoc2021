...
evaluate :: Packet -> Int
evaluate (Literal _ _ value) = value
evaluate (Operator (Hdr _ 0) _ subs) = sum $ map evaluate subs
evaluate (Operator (Hdr _ 1) _ subs) = product $ map evaluate subs
evaluate (Operator (Hdr _ 2) _ subs) = minimum $ map evaluate subs
evaluate (Operator (Hdr _ 3) _ subs) = maximum $ map evaluate subs
evaluate (Operator (Hdr _ 5) _ [p, q]) = if evaluate p > evaluate q then 1 else 0
evaluate (Operator (Hdr _ 6) _ [p, q]) = if evaluate p < evaluate q then 1 else 0
evaluate (Operator (Hdr _ 7) _ [p, q]) = if evaluate p == evaluate q then 1 else 0

evaluateHex h = evaluate packet
    where PR packet _ = parsePacket $ hexToBits h
